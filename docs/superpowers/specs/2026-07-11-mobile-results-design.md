...
Подписывается на `resultsRef.current`. Пороговое значение `threshold: 0` — sticky скрывается как только хотя бы один пиксель блока входит во viewport.
...
Две колонки: левая растягивается, правая фиксирована (шеврон).
...
- **Левая, верхняя строка:** статус — «Участвовать» / «Осторожно» / «Отказаться» / «Недостаточно данных»
- **Левая, нижняя строка:** запас или убыток (`nmccDiff` в ₽, со знаком) — скрыта когда `!isReady`
- **Правая:** шеврон вниз (иконка), выровнен по центру по вертикали
